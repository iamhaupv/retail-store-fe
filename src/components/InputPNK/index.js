...
export default function InputPNK({ suggestion, value, onchange, placeholder }) {
  ...
  const filteredSuggestion =
    query === ""
      ? suggestion
      : suggestion.filter(
          (item) => item.idPNK?.toLowerCase().includes(query.toLowerCase()) // Lọc theo idPNK nếu không có name
        );
          ...
          {filteredSuggestion.length === 0 ? (
            ...
            filteredSuggestion.map((object) => (
              ...
              >
                ...
                <div className="text-sm/6">{object.idPNK || "Unnamed"}</div>{" "}
                {/* Hiển thị "Unnamed" nếu idPNK rỗng */}
              </ComboboxOption>
            ))
          )}
        </ComboboxOptions>
