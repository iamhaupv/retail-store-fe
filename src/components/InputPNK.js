import {
  ...
  export default function InputPNK({ suggestion, value, onchange, placeholder }) {
    ...
    const filteredSuggestion = query === ""
      ? suggestion
      : suggestion.filter((item) =>
          item.idPNK?.toLowerCase().includes(query.toLowerCase()) // Lọc theo idPNK nếu không có name
        );
  
    // Xử lý khi chọn một mục từ danh sách
    const handleSelect = (item) => {
      if (item) { // Kiểm tra item không phải là null hoặc undefined
        setSelected(item);
        setQuery(item.idPNK || ""); // Kiểm tra idPNK trước khi hiển thị
        onchange(item.idPNK || "");  // Cập nhật giá trị cha
      } else {
        setSelected(null);
        setQuery("");
        onchange("");
      ...
      <div className="mx-auto h-full w-full">
        ...
        >
          <div className="relative">
            <ComboboxInput
              className={clsx(
                "w-full rounded-lg border py-1.5 pr-8 pl-3 text-sm text-black",
            ...
            />
            <ComboboxButton className="group absolute inset-y-0 right-0 px-2.5" onClick={() => setQuery("")}>
              <svg
          ...
          <ComboboxOptions
            className={clsx(
              "w-full rounded-xl border bg-white p-1",
          ...
          >
            {filteredSuggestion.length === 0 ? (
              ...
              filteredSuggestion.map((object) => (
                ...
                >
                  <div className="text-sm">{object.idPNK || "Unnamed"}</div> {/* Hiển thị "Unnamed" nếu idPNK rỗng */}
                </ComboboxOption>
              ))
            )}
          </ComboboxOptions>
